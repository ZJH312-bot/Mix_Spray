...
typedef struct
{
   uint8_t Head;                // 帧头 0xAC
   uint8_t Cmd;                 // 命令字
   uint8_t Motor_Control;       // 电机控制命令
   uint16_t spray_motor_current;   // 喷涂电机电流 
   uint16_t mix_motor_current;     // 搅拌电机电流
   uint8_t Auto_continuous_time;// 自动连续搅拌时间
   uint8_t Auto_interval_time;  // 自动搅拌间隔时间
   float Kp;                  // PI控制Kp参数
   float Ki;                  // PI控制Ki参数
   uint8_t sum8;              // 前面17字节的sum8校验和
} RECEIVE_DATA;
...
// 组帧发送
 uint8_t USB_Tx_SendFrame(uint8_t cmd, uint8_t Motor_Control,uint8_t spray_motor_current, uint8_t mix_motor_current, 
                        uint8_t Auto_continuous_time, uint8_t Auto_interval_time,uint8_t GROUND_STATE);

extern System_State_data system_state_data;
extern uint8_t usb_Receive_buf[18];
